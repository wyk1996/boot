...
{ 	
	...
	if(TMR3->STS&0X0001)//溢出中断
	{
		...
		{
			if((curtime >Uart2RecvControl.lastrecvtime) ?((Uart2RecvControl.lastrecvtime - curtime) > CM_TIME_50_MSEC) \
		...
		{
			if((curtime >Uart1RecvControl.lastrecvtime) ?((Uart1RecvControl.lastrecvtime - curtime) > CM_TIME_5_MSEC) \
